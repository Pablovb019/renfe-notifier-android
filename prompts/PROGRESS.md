...
- Pruebas del realme pendientes: confirmar en dispositivo el canal `disponibilidad_plazas_v2` tras instalar v0.1.7 (y que el antiguo desaparece), pantalla apagada, Doze, reinicio, Wi-Fi/datos, ahorro de bateria, cierre desde recientes vs forzar detencion. No se inventaran resultados sin verificacion real.

## Siguiente paso
- Verificacion local del backend (ruff, mypy, pytest) y commit+push del fix de canal + version; aviso: dispara CI. Esperar CI verde y crear/pushear tag v0.1.7 (release APK firmado).
- Instalar v0.1.7 encima y validar en el realme: canal `disponibilidad_plazas_v2` con `usage=USAGE_NOTIFICATION`, canal antiguo eliminado, firma y datos intactos, y notificacion de prueba llegando por el canal nuevo.
- Redeploy del backend en la VM (el usuario ejecuta `git pull` + restart) para que `channel_id` del payload sea el nuevo.
